...
+  Logger.info(`REQUEST→${url}→${method}`, JSON.stringify(params))
  ...
  return req.request(fullUrl, options)
    .then(res => {
      if (res.result) {
        // 响应日志：
+        Logger.info(`REQUEST→${url}→${method}`, res.result.toString().substring(0, 250))
        // 1. 处理响应
        const response = JSON.parse(res.result as string) as {
          code: number
          message: string
    ...
    .catch(err => {
      promptAction.showToast({ message: '网络错误' })
      // 错误日志：
+      Logger.error(`REQUEST→${url}→${method}`, JSON.stringify(err).substring(0, 250))
      return Promise.reject(err)
    })
    .finally(() => {
      // 销毁请求释放资源
...
```typescript type="common/utils/Basic.ts"
